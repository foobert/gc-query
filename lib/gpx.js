...
async function generate(cursor, writer) {
  let count = await cursor.count();
  cursor.rewind();
  debug("Generating GPX from %d documents", count);
  debug("Result streaming is %s", writer ? "enabled" : "disabled");

  let output;
  const write = writer || output.push;

  write(
    '<?xml version="1.0" encoding="UTF-8" standalone="no"?><gpx xmlns="http://www.topografix.com/GPX/1/1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd" version="1.1" creator="cachecache">'
  );
