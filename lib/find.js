...
// Read the number of geocaches for a tile from the gcs collection (gc database)
function geocachesCount(collection, query) {
  const queryDoc = buildLightQueryDoc(query);
  debug(
    "Number of geocaches found based on \n%O\nbuilt from %o",
    queryDoc,
    query
  );
  debug("db.getCollection('gcs').count(%s)", JSON.stringify(queryDoc));
...
// Light query with less filters, to speed up the query
function buildLightQueryDoc(query) {
  let queryDoc = {
    coord: { $exists: true },
    parsed: { $exists: true },
    "parsed.premium": false,
  ...
  // add quadkey filter to query
  Object.assign(queryDoc, filter(query.quadkey, filterQuadKey));
  return queryDoc;
}
...
module.exports = { find, findPagination, geocachesCount };
